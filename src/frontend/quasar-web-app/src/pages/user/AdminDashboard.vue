<template>
  <q-page class="admin-page">
    <q-card>
      ...
      <q-card-section>
        <q-table :columns="columns" :rows="userStore.users" row-key="id">
          <template v-slot:body-cell="props">
            ...
            <q-td :props="props">
              ...
              <template v-else-if="props.col.name === 'role'">
                <q-select
                  v-model="props.row.role"
                  :options="roleOptions"
                  @update:model-value="onRoleChange(props.row, $event)"
                  dense
                />
              </template>
              <template v-else>
...
const COLUMNS = [
  ...
  { name: 'role', required: true, label: 'Role', align: 'left', field: row => row.role }, // Column for the user's role
...
// Extracted function for updating the user's role
const changeUserRole = async (userStore, user, newRole) => {
  user.role = newRole;
  await userStore.updateUser(user); // Call the store action to update a user
};

export default {
  setup() {
    const userStore = useUserStore(); // Initialize the user store
    const roleOptions = ref(['Admin', 'User', 'Viewer']); // Define the available roles

    const onDeleteUser = (userId) => deleteUser(userStore, userId);
    const onRoleChange = (user, newRole) => changeUserRole(userStore, user, newRole);

    onMounted(async () => {
      await userStore.fetchUsers(); // Fetch the list of users when the component is mounted
    });

    return {userStore, columns: COLUMNS, roleOptions, onDeleteUser, onRoleChange}; // Return the store, columns, role options, and delete function to the template
  }
};
</script>
